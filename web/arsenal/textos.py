...
GLIFOS_EN = {
    ...
    'Augurio de claridad': 'Omen of Clarity',
    ...
    'Kilrogg': 'Kilrogg',
    'Lanzamiento destrozador': 'Shattering Throw',
...
def _clave(nombre):
    """Nombre de glifo reducido a lo que lo identifica.

    Las transcripciones no siempre respetan la mayúscula ni el artículo del
    nombre oficial —«Del Sabio» por «Sabio», «Lo Salvaje» por «lo Salvaje»—,
    y por una mayúscula el glifo se quedaba sin traducir y sin icono.
    """
    n = unicodedata.normalize("NFD", (nombre or "").strip().lower())
    n = "".join(c for c in n if unicodedata.category(c) != "Mn")
    return re.sub(r"^(?:del|de la|de|la|el|lo|los|las)\s+", "", n)


_GLIFOS_POR_CLAVE = {}
for _es, _en in GLIFOS_EN.items():
    _GLIFOS_POR_CLAVE.setdefault(_clave(_es), (_es, _en))
for _en, _es in GLIFOS_ES.items():
    _GLIFOS_POR_CLAVE.setdefault(_clave(_en), (_es, _en))


def glifo_par(nombre, idioma):
    """Devuelve (español, inglés) de un glifo, tolerando la transcripción."""
    if not nombre:
        return None
    otro = GLIFOS_EN.get(nombre) if idioma == "es" else GLIFOS_ES.get(nombre)
    if otro:
        return (nombre, otro) if idioma == "es" else (otro, nombre)
    par = _GLIFOS_POR_CLAVE.get(_clave(nombre))
    if par:
        return par
